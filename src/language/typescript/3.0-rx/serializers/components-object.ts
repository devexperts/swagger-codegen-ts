...
const serializeSchema = combineReader(
	...
	serializeSchemaObject => (name: string, schema: OpenAPIV3.SchemaObject, cwd: string): Either<Error, File> => {
				...
				pipe(
					serializeDictionary(schemas, (name, schema) => {
						if (isReferenceObject(schema)) {
							return left(
								new Error(
									'References inside ComponentsObject.schemas dictionary are not supported yet',
								),
							);
						} else {
